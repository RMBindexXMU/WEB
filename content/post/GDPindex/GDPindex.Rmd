...
if(1){
  ...
  ###################################
  ts <- data 
  #ts <- ts["2012-09-01/"]#ts <- rollmean(ts,k=30) * 100
  names(ts) <- c("23国","37国")
  ###################################
  dy.colors <- c("red","blue","green")
dygraph(data = ts , main = " ",width=700,height=400) %>%
    dyRangeSelector(dateWindow = c("2012-09-01", "2019-04-01"), height = 20)  %>%#valueRange = c(min(data)-4, max(data)+4)
    dyAxis("y", label = " ") %>%
    ...
    dyEvent("2013-10-03","提出“一带一路”倡议", labelLoc = "bottom") %>%
