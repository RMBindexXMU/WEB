...
library(dygraphs)
library(xts)
load(file = "data/Rdata/latex_index_i.Rdata")
```
...
if(1){
  ...
  ###################################
  ts <- cbind(index.i45$index.all$CHN,
              index.i26$index.all$CHN,
              index.i45e$index.all$CHN
              ) %>% as.data.frame
  ts <- ts[-c(1:500),]
  ts <- xts(ts, as.Date(index.i45$Date, format='%Y-%m-%d'))
  ts <- ts["2009-01-01/"]#ts <- rollmean(ts,k=30) * 100
  names(ts) <- c("沿线国家货币","参与国货币","参与国货币+欧元")
  ###################################
  dy.colors <- c("red","blue","green")
  dygraph(data = ts , main = " ") %>%
    dyRangeSelector(dateWindow = c("2017-01-01", "2019-04-01"), height = 20)  %>%#valueRange = c(min(data)-4, max(data)+4)
    dyAxis("y", label = " ") %>%
    ...
    dyEvent("2008-09-14", "1", labelLoc = "bottom") %>%#2008.9.14:雷曼兄弟破产
